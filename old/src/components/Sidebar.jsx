  ...
  const getLevelStyles = () => {
    switch (level) {
      ...
      button: `px-4 py-2.5 rounded-xl transition-all duration-200 group ${
        isActive 
          ? 'bg-indigo-700 text-white shadow-xl shadow-indigo-200' 
          : 'text-slate-950 hover:bg-slate-200/50'
      }`,
      icon: `text-lg ${isActive ? 'text-white' : 'text-slate-700 group-hover:text-indigo-700'}`,
      text: `text-sm font-black tracking-tight`,
      };
      case 2:
      return {
      container: 'ml-4 border-l-2 border-slate-300 my-0.5',
      button: `pl-4 pr-4 py-2 rounded-lg transition-all duration-200 group ${
        isActive ? 'bg-indigo-100 text-indigo-800 border-l-4 border-indigo-700 font-black' : 'text-slate-900 hover:bg-slate-100 hover:text-black font-bold'
      }`,
      icon: 'hidden',
      text: 'text-sm',
      };
      case 3:
      return {
      container: 'ml-8 border-l-2 border-slate-200',
      button: `pl-4 pr-4 py-1.5 rounded-lg transition-all duration-200 group ${
        isActive ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600 font-black' : 'text-slate-800 hover:bg-slate-50 hover:text-indigo-800 font-bold'
      }`,
      icon: 'hidden',
      text: 'text-xs',
      };      default: return {};
