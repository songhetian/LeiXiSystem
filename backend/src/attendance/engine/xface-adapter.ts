...
export function parseXFaceRecords(response: XFaceApiResponse): XFaceRecord[] {
  if (!response || typeof response !== 'object') {
    throw new Error('XFace API response is null or invalid');
  }
  if (response.ret !== 0) {
    throw new Error(`XFace API error: ${response.msg || 'ret=' + response.ret}`);
  }
  ...
  for (const row of response.rows) {
    ...
    const deviceNo = row.device_sn || row.deviceSn || row.sn || row.serial_no || 'unknown';
