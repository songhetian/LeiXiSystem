...
import { 
  ...
  ActionIcon, Stack, Tabs, rem, Menu, ThemeIcon
} from '@mantine/core';
import { 
      ...
      <Stack gap="lg">
        <Paper withBorder p="xl" radius="lg" shadow="xs">
          <Group justify="space-between" mb="xl">
            <Box>
              <Title order={3} fw={800}>考试中心</Title>
              <Text size="sm" c="dimmed">统一管理试卷、计划与结果。</Text>
            </Box>
          ...
          <Tabs value={activeTab} onChange={setActiveTab} variant="outline" radius="xl" mb="xl">
            <Tabs.List>
              <Tabs.Tab value="exams" leftSection={<FileText size={16} />}>试卷管理</Tabs.Tab>
              <Tabs.Tab value="plans" leftSection={<Calendar size={16} />}>考核计划</Tabs.Tab>
              <Tabs.Tab value="results" leftSection={<GraduationCap size={16} />}>成绩审计</Tabs.Tab>
            </Tabs.List>
          </Tabs>
          ...
          <Group wrap="nowrap" gap="md" mb="xl">
            ...
            />
            <ActionIcon variant="light" color="blue" size={44} radius="md" onClick={() => refetch()}>
              <RefreshCw size={20} />
            </ActionIcon>
          </Group>

          <Paper withBorder radius="lg" style={{ overflow: 'hidden' }}>
            <LXTable columns={columns} data={data?.data || []} loading={isLoading} pagination={{ current: filters.page, pageSize: 20, total: data?.total || 0, onChange: (p) => setFilters((prev) => ({ ...prev, page: p })) }} />
          </Paper>
        </Paper>
