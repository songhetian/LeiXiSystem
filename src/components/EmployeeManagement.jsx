...
function EmployeeManagement() {
  ...
  const performSubmit = async () => {
    try {
      ...
      if ((await res.json()).success) { toast.success('名册更新成功'); setIsModalOpen(false); fetchEmployees(); resetForm(); }
  ...
  const handleUserDepartmentSuccess = () => { toast.success('数据权限已同步'); fetchEmployees(); };
        ...
        <div className="bg-slate-50/40 px-10 py-8">
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[280px]">
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-1.5 tracking-widest ml-1">检索成员</label>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-100/50">
                ...
                <th className="px-6 py-6 text-center text-[11px] font-black text-slate-900 uppercase tracking-[0.2em]">操作中枢</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-center">
              {getCurrentPageData().length === 0 ? (
                <tr><td colSpan="6" className="py-32 text-center text-slate-900 font-black tracking-widest text-[15px] uppercase">暂无符合条件的成员记录</td></tr>
              ) : (
        ...
        {filteredEmployees.length > 10 && (
          <div className="px-10 py-8 bg-slate-50/50 flex items-center justify-between border-t border-slate-200 rounded-b-2xl shadow-inner">
              <div className="flex items-center gap-4 text-left font-black">
                  <span className="text-[12px] font-black text-slate-900 uppercase tracking-widest">共管理 <span className="text-indigo-600">{filteredEmployees.length}</span> 名成员</span>
        ...
        <div className="space-y-4 font-black text-left">
          {statusChangingEmp && (
            <div className="p-4 bg-slate-50 border-[1px] border-slate-300 rounded-xl text-sm">
              <p><span className="text-slate-400 uppercase text-[10px] block mb-1">目标成员</span><span className="text-slate-900 font-black">{statusChangingEmp.real_name} (工号: {statusChangingEmp.employee_no})</span></p>
            </div>
          )}
