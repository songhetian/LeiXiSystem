...
const UserRoleManagement = () => {
  ...
  const columns = [
    ...
    { title: '组织架构', dataIndex: 'department_name', key: 'dept', align: 'center', render: (t) => <span className="text-[13px] font-black text-slate-700 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100">{t || '未分配部门'}</span> },
    { title: '当前权限角色', key: 'roles', align: 'center', render: (_, r) => (
        <div className="flex flex-wrap gap-1.5 justify-center">
            {r.roles?.length > 0 ? r.roles.map(role => (
                <Tag key={role.id} variant="borderless" className={`m-0 font-black text-[11px] px-2.5 py-0.5 rounded-md border ${role.name === '超级管理员' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-indigo-50 text-indigo-600 border-indigo-100'}`}>
    ...
    { title: '数据可见性', key: 'scope', align: 'center', render: (_, r) => (
        <div className="flex flex-wrap gap-1 justify-center">
            {r.departments?.length > 0 ? r.departments.slice(0, 2).map(d => <span key={d.id} className="px-2.5 py-0.5 bg-purple-50 text-purple-700 rounded text-[11px] font-black border border-purple-100">{d.name}</span>) : <span className="text-[11px] font-bold text-slate-400">仅本人数据</span>}
    ...
    { title: '决策中心', key: 'action', align: 'center', width: 200, render: (_, r) => (
        <div className="flex items-center justify-center gap-1">
            <button onClick={() => handleManageRoles(r)} className="px-4 py-2 text-[11px] font-black text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all border border-indigo-50">角色变更</button>
            <button onClick={() => { setSelectedUserForDepartment(r); setIsDepartmentModalOpen(true); }} className="px-4 py-2 text-[11px] font-black text-purple-600 hover:bg-purple-50 rounded-lg transition-all border border-purple-50">可见范围</button>
        </div>
    )}
  ];
  ...
  const renderPageNumbers = () => {
    ...
    for (let i = start; i <= end; i++) pages.push(<button key={i} onClick={() => handlePageChange(i)} className={`w-10 h-10 rounded-lg text-sm font-black transition-all ${currentPage === i ? 'bg-slate-900 text-white shadow-lg scale-110' : 'bg-white border border-slate-200 text-slate-600'}`}>{i}</button>);
    return pages;
  };
    ...
    <div className="p-6 bg-[#f8fafc] min-h-screen select-none animate-in fade-in duration-500 text-slate-900 text-left">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm mb-6 overflow-hidden">
        <div className="flex items-center justify-between gap-4 px-10 py-6 border-b border-slate-50">
          ...
          <button onClick={fetchUsers} className="h-11 w-11 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-all shadow-inner"><RefreshCcw size={18} /></button>
        </div>
        ...
        <div className="bg-slate-50/40 px-10 py-8">
            <div className="flex items-center gap-4 max-w-5xl">
                ...
                <button onClick={() => { setDisplaySearchText(''); setSearchDepartment(''); setCurrentPage(1); }} className="h-11 px-8 bg-indigo-50 text-indigo-600 text-xs font-black rounded-lg hover:bg-indigo-100 transition-all flex items-center gap-2 border border-indigo-100"><X size={14} /> 重置</button>
            </div>
        </div>
      </div>
      ...
      {selectedUserIds.length > 0 && (
        <div className="mb-4 p-4 bg-slate-900 rounded-xl flex items-center justify-between px-10 animate-in slide-in-from-top-4 shadow-xl">
            ...
            <div className="flex gap-2">
                <button onClick={() => setIsBatchAssignOpen(true)} className="h-9 px-6 bg-indigo-600 text-white font-black rounded-lg text-[11px] hover:bg-indigo-500 transition-all shadow-lg">批量分配角色</button>
                <button onClick={() => setIsBatchRemoveOpen(true)} className="h-9 px-6 bg-rose-600 text-white font-black rounded-lg text-[11px] hover:bg-rose-500 transition-all shadow-lg">批量注销权限</button>
        ...
        {filteredUsers.length > 10 && (
          <div className="px-10 py-8 bg-slate-50/50 flex items-center justify-between border-t border-slate-200 rounded-b-2xl">
              <div className="flex items-center gap-4 text-left">
                  ...
                  <Select size="small" value={pageSize} onChange={setPageSize} variant="borderless" className="bg-white rounded-lg shadow-sm border border-slate-300 text-[12px] font-black text-slate-900 w-24" options={[10, 20, 50].map(v => ({ label: `${v} 条`, value: v }))} />
              </div>
      ...
      <Modal title={`级联授权 - ${selectedUser?.real_name}`} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={480} centered>
        <div className="py-6 text-left space-y-4 font-black">
            <label className="text-[11px] font-black text-slate-400 uppercase ml-1 tracking-widest">请指定目标职能角色</label>
            <Select placeholder="搜索系统角色..." value={selectedRoles[0] || null} onChange={v => setSelectedRoles(v ? [v] : [])} className="w-full h-12 font-black" size="large" allowClear>
                {roles.map(r => <Option key={r.id} value={r.id}><div className="flex items-center justify-between font-black"><span>{r.name}</span>{r.is_system && <span className="text-[9px] font-black bg-rose-50 text-rose-600 px-1.5 py-0.5 rounded border border-rose-100">核心预置</span>}</div></Option>)}
            </Select>
            <div className="pt-4 flex gap-2">
                ...
                <button onClick={handleSaveRoles} className="flex-1 h-11 bg-slate-900 text-white font-black rounded-lg hover:bg-black shadow-lg transition-all uppercase text-xs">确认保存</button>
            </div>
        </div>
      </Modal>
      ...
      <Modal title="批量执行角色分配" open={isBatchAssignOpen} onCancel={() => setIsBatchAssignOpen(false)} footer={null} width={480} centered>
        <div className="py-6 text-left space-y-4 font-black">
            <div className="p-4 bg-blue-50 rounded-xl border border-blue-100 text-blue-800 text-xs shadow-inner">已锁定 {selectedUserIds.length} 名成员进行批量处理</div>
            ...
            <div className="pt-4 flex gap-2">
                ...
                <button onClick={handleBatchAssignRoles} disabled={isProcessingBatch} className="flex-1 h-11 bg-indigo-600 text-white font-black rounded-lg hover:bg-indigo-700 shadow-lg transition-all">执行授权</button>
            </div>
        </div>
      </Modal>
      ...
      <Modal title="批量清理权限关联" open={isBatchRemoveOpen} onCancel={() => setIsBatchRemoveOpen(false)} footer={null} width={480} centered>
        <div className="py-6 text-left space-y-4 font-black">
            ...
            <div className="p-4 bg-rose-50 text-rose-700 text-xs font-black rounded-xl border border-rose-100 leading-relaxed">此操作将清空目标成员的所有职能角色，导致其无法访问任何受保护资源。</div>
            <div className="pt-4 flex gap-2">
                ...
                <button onClick={handleBatchRemoveRoles} disabled={isProcessingBatch} className="flex-1 h-11 bg-rose-600 text-white font-black rounded-lg hover:bg-rose-700 shadow-lg transition-all">确认注销</button>
            </div>
        </div>
      </Modal>
