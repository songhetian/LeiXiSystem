...
import { Card, Button, Space, message, Spin, Progress, Modal, Typography, Radio, Checkbox, Input, InputNumber } from 'antd';
import { ArrowLeftOutlined, CheckOutlined, FlagOutlined, ClockCircleOutlined } from '@ant-design/icons';
...
const ExamTaking = () => {
  ...
  const saveAnswerToBackend = useCallback(async (resId, payload) => {
    const { questionId, answer } = payload || {};
    if (!resId || !questionId) return;
    try {
      await axios.put(`/api/assessment-results/${resId}/answer`, {
  ...
  const debouncedSaveAnswer = useAutoSave(saveAnswerToBackend, resultId, 3000);
  ...
  const handleAnswerChange = (questionId, answer) => {
    ...
    debouncedSaveAnswer({ questionId, answer: formattedAnswer });
  ...
  const handleSubmitExam = async (isTimeout = false) => {
    ...
    debouncedSaveAnswer.cancel(); // Cancel any pending auto-saves
        ...
        <Card style={{ width: '100%', position: 'fixed', top: 0, zIndex: 100, borderRadius: 0 }} bodyStyle={{ padding: '10px 24px' }}>
          <Space style={{ width: '100%', justifyContent: 'space-between' }}>
            ...
            <Space>
              ...
              <Skeleton.Progress size="small" style={{ width: 120 }} active />
  ...
  const renderAnswerInput = (question) => {
    ...
    try {
      ...
      switch (question.type) {
        ...
        case 'fill_blank':
          // Assuming fill_blank questions have multiple blanks, represented by an array of inputs
          // For simplicity, let's assume a single input for now, or multiple if question.content indicates
          // A more robust solution would parse question.content for blanks
          const blankCount = (question.content.match(/____/g) || []).length || 1; // Count blanks or default to 1
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, paddingTop: 64, overflow: 'hidden' }}> {/* Adjust paddingTop for fixed header */}
        {!isMobile && (
          <Card style={{ width: 200, flexShrink: 0, overflowY: 'auto', borderRadius: 0 }}>
            <QuestionNav
...
const ExamTaking = () => {
  ...
  const handleAnswerChange = (questionId, answer) => {
    const formattedAnswer = typeof answer === 'object' ? JSON.stringify(answer) : String(answer);
    setUserAnswers((prev) => ({ ...prev, [questionId]: formattedAnswer }));
    debouncedSaveAnswer(questionId, formattedAnswer);
    sendLog('ANSWER_CHANGED', { questionId, answer: formattedAnswer });
  };
