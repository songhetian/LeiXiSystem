...
module.exports = async function (fastify, opts) {
    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO quality_cases
         (title, category, case_type, difficulty_level, priority,
          problem_description, solution, key_points, session_id,
          status, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [title, category, case_type || 'excellent', difficulty_level || 'medium',
         priority || 'medium', problem_description, solution, key_points,
  ...
  fastify.get('/api/quality/cases', async (request, reply) => {
    ...
    // 搜索
    if (search) {
      conditions.push('(qc.title LIKE ? OR qc.problem_description LIKE ? OR qc.solution LIKE ?)');
    ...
    // 难度筛选
    if (difficulty) {
      conditions.push('qc.difficulty_level = ?');
    ...
    try {
      ...
      const [result] = await connection.query(
        `UPDATE quality_cases SET
          title = ?, category = ?, case_type = ?, difficulty_level = ?,
          priority = ?, problem_description = ?, solution = ?, key_points = ?,
          session_id = ?, status = ?, updated_by = ?
         WHERE id = ?`,
        [title, category, case_type, difficulty_level, priority,
         problem_description, solution, key_points, related_session_id,
         status, user_id, id]
      );
