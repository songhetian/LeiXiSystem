...
module.exports = async function (fastify, opts) {
  ...
  fastify.get('/api/todo/list', async (request, reply) => {
    ...
    try {
      ...
      if (isNaN(uid)) {
        return reply.code(400).send({ success: false, message: 'Invalid user_id' });
      }
      ...
      const [approverConfig] = await pool.query(
        'SELECT custom_type_name FROM approvers WHERE user_id = ? AND is_active = 1', [uid]
      )
      const customTypeNames = (approverConfig || []).map(a => a.custom_type_name)

      // --- 1. 获取报销申请 (基于灵活流程引擎) ---
      // 这里的 SQL 采用参数绑定形式改写，防止拼串失败
      const myReimbursementSQL = `
        ...
        WHERE r.status = 'approving'
        AND EXISTS (
          SELECT 1 FROM approval_workflow_nodes n
          WHERE n.id = r.current_node_id
          AND (
            (n.approver_type = 'user' AND n.approver_id = ?) OR
            (n.approver_type = 'role' AND n.role_id IN (${roleIds.length > 0 ? roleIds.map(() => '?').join(',') : '-1'})) OR
            (n.approver_type = 'custom_group' AND n.custom_type_name IN (${customTypeNames.length > 0 ? customTypeNames.map(() => '?').join(',') : "''"})) OR
            (n.approver_type = 'initiator' AND r.user_id = ?) OR
            (n.approver_type = 'department_manager' AND r.department_id = (SELECT department_id FROM users WHERE id = ? AND is_department_manager = 1))
          )
        )
      `
      const rbParams = [uid, ...roleIds, ...customTypeNames, uid, uid];
      const [reimbursements] = await pool.query(myReimbursementSQL, rbParams);
      ...
      let attendanceTasks = []
            if (permissions.canViewAllDepartments || permissions.roles.some(r => r.name.includes('主管') || r.name.includes('经理'))) {
              ...
              const [leaves] = await pool.query(`
                SELECT l.id, 'LEAVE' as no, CONCAT(l.leave_type, ' ', l.days, '天') as summary, 
              ...
              const [overtimes] = await pool.query(`
                SELECT o.id, 'OVERTIME' as no, CONCAT('加班', ' ', o.hours, '小时') as summary, 
      ...
      console.error('❌ [Todo Center] 获取待办列表失败:', error);
